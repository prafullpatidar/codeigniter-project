...
Date.prototype.getWeek = function() {
	var onejan = new Date(this.getFullYear(), 0, 1);
	var weekNo = Math.ceil(( ( (this.getTime() - onejan.getTime()) / 86400000) + 1)/7);
	return weekNo;
};
...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
			...
			if(s.getHours() > 12){
				e.start_hour = (s.getHours()-12) + ':' + s.getMinutes().toString().formatNum(2)+'PM';
			}else{
				e.start_hour = s.getHours().toString().formatNum(2) + ':' + s.getMinutes().toString().formatNum(2)+'AM';
			}

			if(f.getHours() > 12){
				e.end_hour = (f.getHours()-12) + ':' + f.getMinutes().toString().formatNum(2)+'PM';
			}else{
				e.end_hour = f.getHours().toString().formatNum(2) + ':' + f.getMinutes().toString().formatNum(2)+'AM';
			}
	...
	Calendar.prototype._hour = function(hour, part) {
		...
		var frmt = (h >= 12)?'PM':'AM';
		return (h > 12)?((h-12) + ":" + m+frmt):(h + ":" + m+frmt);
	};
	...
	Calendar.prototype.navigate = function(where, next) {
		...
		this.options.events_source = base_url+'calendar/event_list/'+this.options.view+'/'+this.options.day+'/'+this.options.calendar_contact_id;
		this.view();
	...
	Calendar.prototype._init_position = function() {
		...
		this.options.events_source = base_url+'calendar/event_list/'+this.options.view+'/'+this.options.day+'/'+this.options.calendar_contact_id;
		console.log(base_url+'calendar/event_list/'+this.options.view+'/'+this.options.day+'/'+this.options.calendar_contact_id);
		return this;
	};
	...
	Calendar.prototype._update = function() {
		...
		$('*[data-cal-date]').click(function() {
			...
			$.ajax({
                type: "POST",
                url: base_url +'calendar/calendar_task_event/'+calendar_contact_id,
                cache:false,
                data: '',
