...
(function (factory) {
  ...
  var Context = function ($note, options) {
    ...
    this.createInvokeHandler = function (namespace, value) {
      return function (event) {
        ...
        var edtr_lst_itm = namespace.split(".").pop(-1);
        if(edtr_lst_itm == 'fontName'){
          $('.dropdown-fontname li a').removeClass('checked');
          //var font_name = ['Roboto','ArialMT', 'Arial-Black', 'ComicSansMS', 'CourierNewPSMT','HelveticaNeueRegular', 'Helvetica', 'Impact', 'LucidaGrande','Tahoma', 'Times-Roman', 'Verdana-Bold'];
          var font_name = ['Arial','Courier New','Times New Roman','Verdana','Georgia'];
          if(font_name.includes($(event.target).data('value'))){          
            $('.dropdown-fontname li a').removeClass('checked');
            $(event.target).addClass('checked');
            //$('.note-current-fontname').text($(event.target).data('value'));
            $(event.target).parents('.note-fontname').find('.note-current-fontname').text($(event.target).data('value'));
          }
        }
        //var edtr_lst_itm = namespace.split(".").pop(-1);
        if(edtr_lst_itm == 'fontSize'){
          $('.dropdown-fontsize li a').removeClass('checked');
          $(event.target).addClass('checked');
          $(event.target).parents('.note-fontsize').find('.note-current-fontsize').text($(event.target).data('value'));
        }
  ...
  $.fn.extend({
    ...
    summernote: function () {
      ...
      if ($note.length) {
        ...
        ///////code for by default apply 12px font size/////
        // setTimeout(function(){
        //   context.invoke('editor.fontSize', 12);
        // },500); 
  ...
  var dialog = renderer.create('<div class="modal textarea_img_popup" aria-hidden="false"/>', function ($node, options) {
    $node.html([
      '<div class="modal-dialog">',
      '  <div class="modal-content">',
      (options.title ?
      '    <div class="modal-header">' +
      '      <button type="button" class="close close_textarea_img_popup" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
  ...
  $.extend($.summernote.lang, {
    'en-US': {
      ...
      video: {
        ...
        insert: 'Insert Video',
        url: 'Video URL',
        providers: '(YouTube, Vimeo, Vine, Instagram, DailyMotion, Youku or if you have a video then upload it in Digital Library and insert that video url.)'
      },
      link: {
        link: 'Link',
        insert: 'Insert Link',
      ...
      options: {
        help: 'Help', //fullscreen: 'Full Screen',
        codeview: 'Code View'
      },
      paragraph: {
        paragraph: 'Paragraph',
  ...
  var Style = function () {
    ...
    this.fromNode = function ($node) {
      ...
      if(styleInfo['font-family'] == 'walkme-widget-font') {
        styleInfo['font-size']='13px';
      }
  ...
  var Buttons = function (context) {
    ...
    this.addToolbarButtons = function () {
        ...
        return ui.buttonGroup([
          ...
          ui.dropdownCheck({
            ...
            items: options.fontNames.filter(function (name) {
              return name;//agent.isFontInstalled(name) ||
      ...
      //Hide FullScreen Button from Summernote editor starts here
      ...
      //Hide FullScreen Button from Summernote editor ends here
    ...
    this.updateCurrentStyle = function () {
      ...
      if (styleInfo['font-family'])  {
        var fontNames = styleInfo['font-family'].split(',').map(function (name) {
          var tmp_name = name.replace(/[\'\"]/g, '')
            .replace(/\s+$/, '')
            .replace(/^\s+/, '');
          if(tmp_name=='Arial' || tmp_name=='Courier New' || tmp_name=='Times New Roman' || tmp_name=='Verdana' || tmp_name=='Georgia'){
          return tmp_name;}else{
              return '-Select Font-';
            }
        });
        ...
        if(fontName == 'walkme-widget-font'){
          fontName='Roboto';
        }
    ...
    this.showImageDialog = function () {
      return $.Deferred(function (deferred) {
        ...
        ui.onDialogShown(self.$dialog, function () {
          ...
          $imageUrl.on('keyup paste keypress', function () {
  ...
  var VideoDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group row-fluid">' +
          ...
          '<small class="text-muted">' + lang.video.providers + '</small></div>';
      ...
      return $.Deferred(function (deferred) {
        ...
        ui.onDialogShown(self.$dialog, function () {
          ...
          $videoUrl.val(text).on('input', function () {
            ui.toggleBtn($videoBtn, $videoUrl.val().trim());
          }).trigger('focus');

          $videoBtn.click(function (event) {
            event.preventDefault();
            deferred.resolve($videoUrl.val().trim());
          });
  ...
  $.summernote = $.extend($.summernote, {
    ...
    options: {
      ...
      // fontNames: ['Roboto',
      //   'ArialMT', 'Arial-Black', 'ComicSansMS', 'CourierNewPSMT',
      //   'HelveticaNeueRegular', 'Helvetica', 'Impact', 'LucidaGrande',
      //   'Tahoma', 'Times-Roman', 'Verdana-Bold'
      // ],
      fontNames: ['Arial','Courier New','Times New Roman','Verdana','Georgia'],
      // fontNames: ['Roboto',
...
/////////////////below code for extra operations perform on image///////////
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    define(['jquery'], factory);
  } else if (typeof module === 'object' && module.exports) {
